...
function get_mousepos(window::RenderWindow)
    return ccall((:sfMouse_getPosition, libcsfml_window), Vector2i, (Ptr{Cvoid},), window.ptr)
end

function set_mousepos(pos::Vector2, window::RenderWindow)
    pos = to_vec2i(pos)
    ccall((:sfMouse_setPosition, libcsfml_window), Cvoid, (Vector2i, Ptr{Cvoid},), pos, window.ptr)
end
